...
<script lang="ts">
  ...
  function getToastStyles(type: string) {
    const styles = {
      success: {
        container: 'bg-green-50 dark:bg-green-900/20 text-green-900 dark:text-green-100 border-green-200 dark:border-green-800',
        icon: 'text-green-600',
        iconPath: 'M5 13l4 4L19 7',
        progressBar: 'bg-green-600'
      },
      error: {
        container: 'bg-red-50 dark:bg-red-900/20 text-red-900 dark:text-red-100 border-red-200 dark:border-red-800',
        icon: 'text-red-600',
        iconPath: 'M6 18L18 6M6 6l12 12',
        progressBar: 'bg-red-600'
      },
      warning: {
        container: 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-900 dark:text-yellow-100 border-yellow-200 dark:border-yellow-800',
        icon: 'text-yellow-600',
        iconPath: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
        progressBar: 'bg-yellow-600'
      },
      info: {
        container: 'bg-blue-50 dark:bg-blue-900/20 text-blue-900 dark:text-blue-100 border-blue-200 dark:border-blue-800',
        icon: 'text-blue-600',
...
>
  <div class="flex flex-col gap-2 items-end">
    {#each toast.toasts as toastItem (toastItem.id)}
      ...
      >
        <div 
          class="flex items-start gap-3 px-4 py-3 rounded-lg shadow-lg border dark:border-opacity-50 backdrop-blur-sm 
